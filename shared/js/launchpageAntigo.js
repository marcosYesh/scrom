...
/*******************************************************************************
 * Further content definition -associates pages with learning objectives and
 * tracks which pages have been visited -tracks the questions reported by
 * learning objective
 ******************************************************************************/
...
pageObjectiveArray[14] = new ObjectiveCompletion("", false);  // no objective
																// for the final
																// test
...
function doStart(){
    ...
    SetupIFrame();
    // record the time that the learner started the SCO so that we can report
	// the total time
    ...
    if (bookmark == ""){
        ...
        // if there is a stored bookmark, prompt the user to resume from the
		// previous location
...
function goToPage(){
    ...
    // in this sample course, the course is considered complete when the last
	// page is reached
...
function RecordTest(score){
    ...
    /*
	 * In this example, the satisfaction status is derrived from score's
	 * relation to the mastery score so don't explicitly set a score.
	 */
    // consider 70% to be passing
    /*
	 * if (score >= 70){ ScormProcessSetValue("cmi.success_status", "passed"); }
	 * else{ ScormProcessSetValue("cmi.success_status", "failed"); }
	 */
    ...
    for (var objId in objectiveQuestionCount){
        ...
        // also report raw, min and max score as the number of questions correct
		// and available
        ...
        // This example doesn't make any judgement about the satisfaction of
		// each objective. That could be
        // handled by defining a min normalized measure in the manifest or by
		// expliciting setting satisfaction
        // status here.
    }
...
function ConvertMilliSecondsIntoSCORM2004Time(intTotalMilliseconds){
  var ScormTime = "";
  var HundredthsOfASecond;	// decrementing counter - work at the hundreths of a
							// second level because that is all the precision
							// that is required
  var Seconds;	// 100 hundreths of a seconds
  var Minutes;	// 60 seconds
  var Hours;	// 60 minutes
  var Days;		// 24 hours
  var Months;	// assumed to be an "average" month (figures a leap year every 4
				// years) = ((365*4) + 1) / 48 days - 30.4375 days per month
...
// called from the assessmenttemplate.html page to record the results of a
// question
function RecordQuestion(id, questionText, questionType, learnerResponse, correctAnswer, wasCorrect, objectiveId){
    // Interactions are stored in a collection. We need to find the number of
	// items currently in
    // the collection so we can store this question in the next bucket.
    // Since the collection indicies are 0-based, the count will be the
	// identifier for the next
    // available bucket.
    ...
    // Associate this question with one of our four defined learning ojectives
	// to enable
    ...
    // All interaction responses have a very specific format they must ahere to
	// based
    // on the interaction type. In this example, only multiple choice reponses
	// need to
    ...
    // increment the counters tracking the number of questions for each
	// objective
...
/*
 * Marks the current page as visited and updates the progress information for
 * the learning objective associated with this page.
 */
function UpdateLearningObjectivesProgress(currentPage){
    ...
    if (affectedObjective != ""){   // the test doesn't have a learning
        ...
        // If all pages have been completed, mark this learning objective as
		// completed,
        // otherwise, if it's the first time a page has been visited for this
		// objective,
...
function RoundToPrecision(number, significantDigits){
  number = parseFloat(number);
    ...
    /*
	 * Search the objectives collection to find the objective with the specified
	 * id. Since we declared these objectives in the manifest, the LMS should
	 * initialize this collection for us. If they were not declare in the
	 * manifest, the SCO would need to initialize them through runtime calls.
	 */
